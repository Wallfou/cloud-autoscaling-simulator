...
simplified cloud-style workload in \emph{discrete time}.
...
provisioned server-time so different policies can be compared without real
infrastructure.
\end{infocard}
...
Understanding how servers dynamically respond to changing demand connects
directly to how platforms such as AWS, Azure, and GCP operate at scale,
so this project is grounded in a real systems problem rather than a purely
academic one.
It also came from a personal interest in distributed systems.
Building the simulation from scratch made it easier to understand queueing,
resource allocation, and server coordination at a deeper level than simply
reading about them.
The project was also a strong fit for object-oriented design, since clocks,
queues, servers, balancers, and scalers naturally map to separate components
...
\textbf{Contribution:} Supplies variable load so queueing, balancing, and scaling actually matter.
\end{funccard}
...
\textbf{Contribution:} Models waiting work and connects traffic to scaling decisions.
\end{funccard}
...
\textbf{Contribution:} Connects policy settings to measurable outcomes.
\end{funccard}
...
The simulator follows a classical \textbf{Input $\to$ Process $\to$ Output}
...
\begin{enumerate}[label=\textcolor{NURed}{\textbf{D\arabic*.}}, leftmargin=2em]
  \item \textbf{Strategy object for balancing:}
    ...
    with different stategies by swapping the concrete object in
    \texttt{main}.
  ...
  \item \textbf{Struct configuration:}
    ...
    the experiment setup small and readable.
...
The Fast Cooldown policy performed best overall.
It completed the most requests, had the lowest average wait time, and produced
the highest throughput.
That improvement came at the highest cost, since faster reactions allowed the
cluster to spin up capacity more often and accumulate more provisioned time.

The Conservative policy was the cheapest, but it also had the worst
performance.
...
The Aggressive policy improved over the default by reacting earlier to
queue buildup, while the Default policy generally fell between
aggressive and conservative on all metrics.

\subsection{Arrival Pattern Behavior}

Across constant, sine, and burst arrival modes, the simulator showed that
threshold-based autoscaling is fundamentally reactive.
Queue depth often rises first, then the cluster catches up after scale-out.
This lag is especially visible under sin shaped traffic and burst windows,
